...
function collectTasks (levels, fileHash, options) {
  ...
  for (const level of levels) {
    ...
    tasks.push([() => processLevel(options, level, levelFile)])
    tasks.push(level.tiles.map(tile => () => processTile(options, level, levelFile, tile)))
  }
  return tasks
}
...
class MapSlicer extends EventEmitter {
  ...
  async start (options) {
    ...
    try {
      ...
      const imageSize = await getImageSize(options.gm, options.file)
